    ...
    const updateUserName = async () => {
        try {
            const response = await fetch(
                "http://localhost:3000/api/user/usernames"
            );
            ...
            if (isNameExisting) {
                ...
                enqueueSnackbar("Username must be atleast 3 characters long", { variant: "warning" });
                return;
            } else if (edittedName.includes(' ')) {
                enqueueSnackbar("Username should not contain spaces", { variant: "warning" });
                return;
            }
        } catch (error) {
